...
function creatureSayCallback(cid, type, msg)
  ...
  msg = string.lower(msg)
		...
		elseif talk_state == 1 and msgcontains(msg, 'yes') then
			   npcHandler:say('I thought so too. It’s made out of a quite complex ritual.')
			talk_state = 99

		elseif talk_state == 1 and msgcontains(msg, 'no') then
			   npcHandler:say('How arrogant!')
			talk_state = 0
		...
		elseif talk_state == 2 and msgcontains(msg, 'no') then
			   npcHandler:say('No? Okey then.')

		elseif talk_state == 2 and msgcontains(msg, 'yes') then
			   npcHandler:say('I fear I need some important ingredients for my blessing spell first. ...')
				story = 1
				talk_start = os.clock()
				talk_story = os.clock()
		...
		elseif talk_state == 3 and msgcontains(msg, 'yes') then
			   npcHandler:say('Thank you, I promise that your efforts won’t be in vain! Listen closely now. First, I need a sample of each druid rod and sorcerer wand there is in the Lost Lands. Please bring them all at once so that their energy will be balanced. ...')
				story = 2
				talk_start = os.clock()
				talk_story = os.clock()
...
elseif talk_state == 4 and msgcontains(msg, 'no') then
		...
		elseif talk_state == 4 and msgcontains(msg, 'yes') then
			   npcHandler:say('Alright then. Come back to with a sample of all five wands and five rods, please.')
...
elseif talk_state == 20 and msgcontains(msg, 'no') then
		...
		elseif talk_state == 20 and msgcontains(msg, 'yes') then
			...
			setPlayerStorageValue(cid,7321,1)
			   npcHandler:say('Thank you, that must have been a lot to carry. Now, please bring me 10 ounces of magic sulphur.')
			talk_state = 0
			else
...
elseif talk_state == 27 and msgcontains(msg, 'no') then
		...
		elseif talk_state == 27 and msgcontains(msg, 'yes') then
			...
			setPlayerStorageValue(cid,7322,1)
			   npcHandler:say('Very good. I will immediately start to prepare the ritual and extract the elemental energy from the wands and rods. Please bring me the Necromancer\'s soul stone now.')
			talk_state = 0
			else
			selfSay('Come back when you got it!')
			end
...
elseif talk_state == 28 and msgcontains(msg, 'no') then
		...
		elseif msgcontains(msg, 'outfit') or msgcontains(msg, 'addon') and value6 == 1 then
			   npcHandler:say('You already  recieved my wand.')
