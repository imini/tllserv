...
-- Briefing 
 ...
 selfSay('Do you mean you would like to become a applicant member?')
 talk_state = 1
 
 elseif talk_state == 1 and msgcontains(msg,'no') then
 selfSay('Because a you can\'t become a member without becoming an applicant member of the first grade, first.')
 talk_state = 0
 
 elseif talk_state == 1 and msgcontains(msg,'yes') then
 selfSay('So, you would like to become an applicant member of the first grade for the Celerian Academy?')
 talk_state = 2
 ...
 selfSay('Terrific. Then I grant you the title of applicant member of the first grade. Whenever you want, we can proceed in order to promote you to member of the first grade.')
 ...
 selfSay('To rise in rank and become a real member you will have to go through three steps. These there steps are a test of knowledge, a test of stamina and finally a briefing of the rules. Would you like to continue with these?')
 talk_state = 3
 
 elseif talk_state == 3 and msgcontains(msg,'no') then
 selfSay('Return later, if it is so that you feel like doing it then.')
 talk_state = 0
 
 elseif talk_state == 3 and msgcontains(msg,'yes') then
 selfSay('Brilliant! First, I suggest you to go to the nearby Illuminatis library to study a little, if you want to pass the test of knowledge. I hereby grant you access to that library. When you feel ready, return and ask me for the test.')
 setPlayerStorageValue(cid,2001,-1)
 ...
 selfSay('You are still an applicant member of the first grade until you pass the tests.')
 talk_state = 0
 
 elseif value2 == 1 and msgcontains(msg,'test') then
 selfSay('Ah, so you feel prepared to do the test of knowledge?')
 talk_state = 4
 
 elseif talk_state == 4 and msgcontains(msg,'yes') then
 selfSay('Okey. But first, before we begin, I must ask you for the members fee of 250 gold coins. Are you willing to pay that?')
 talk_state = 5
	
 elseif talk_state == 4 and msgcontains(msg,'no') then
 selfSay('Very well, we have all time in the world.')
 talk_state = 0
...
-- Test I  
 ...
 selfSay('You have not brought enough money.')
 talk_state = 0
 end
 
 elseif talk_state == 6 then
 if msgcontains(msg,'stallion') then
 selfSay('The first question answered perfectly well. But do you know the second? What was the name of the person who founded the Celerian Academy?')
 talk_state = 7
 else
 selfSay('I\'m afraid that is wrong. Please return later when you have refreshed your memory.')
 talk_state = 0
 end
 
 elseif talk_state == 7 then
 if msgcontains(msg,'celero') then
 selfSay('Also correct. But during what war was the Academy on the brink of disbanding?')
 talk_state = 8
 else
 selfSay('I\'m afraid that is wrong. Please return later when you have refreshed your memory.')
 talk_state = 0
 ...
 selfSay('Terrific, ' .. getPlayerName(cid) .. '. Now, what is the name of the highest rank possible, except for Stallions?')
 talk_state = 9
 else
 selfSay('I\'m afraid that is wrong. Please return later when you have refreshed your memory.')
 talk_state = 0
 ...
 if msgcontains(msg,'the war of paschendale') then
 selfSay('Indeed, you exceed my expectations. You have soon completed the test. There\'s two questions left. Who led the revolt against king Celero II and the Academy?')
 talk_state = 10
 else
 selfSay('I\'m afraid that is wrong. Please return later when you have refreshed your memory.')
 talk_state = 0
 ...
 selfSay('That is indeed correct. Now, the last question: what is the Celerian Academys motto?')
 talk_state = 11
 else
 selfSay('I\'m afraid that is wrong. Please return later when you have refreshed your memory.')
 talk_state = 0
 ...
 selfSay('I couldn\'t have answered it better myself. Congratulations, my dear ' .. getPlayerName(cid) .. ', you have successfully completed the first test. Now when you feel like continuing, ask me for another one.')
 talk_state = 0
 setPlayerStorageValue(cid,2002,-1)
 ...
 selfSay('Fine. But without money you won\'t be able to live up to the expectations here.')
 talk_state = 0  
...
-- Test II            
 ...
 selfSay('Well, you have completed the first test. This was to put your knowledge to test. Now, we are going to see if you can live up to another ideal personality which the members of the Celerian Academy is presumed to have.. ...')
 story = 2
 talk_start = os.clock()
 talk_story = os.clock()
 ...
 selfSay('Very sad to hear. You are barely living up to the expectations.')
 talk_state = 0
 
 elseif talk_state == 12 and msgcontains(msg,'yes') then
 selfSay('Good. Then go to the Vast Lands in the west and seek this ancient prophecy, but be as quick as you can. Time is valuable.')
 setPlayerStorageValue(cid,2003,-1)
 ...
 selfSay('Is it true that you have found the prophecy?')
 talk_state = 13
 
 elseif talk_state == 13 and msgcontains(msg,'no') then
 selfSay('Come back when you have found it, please.')
 talk_state = 0
 
 elseif talk_state == 13 and msgcontains(msg,'yes') then
 if getPlayerItemCount(cid,xxxx) >= 1 then
 selfSay('Amazing. It\'s really the one! Well, this has to be rewarded. You are on the brink of becoming a member of the first grade, dear ' .. getPlayerName(cid) .. '. Now, everything that is left is the little briefing of rules, but I suppose it won\'t be troublesome.')
 setPlayerStorageValue(cid,2004,-1)
 ...
 selfSay('You are soon a member of the Celerian Academy, ' .. getPlayerName(cid) .. '.')
 talk_state = 0
